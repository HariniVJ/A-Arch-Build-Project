...
const apiUrl = "http://localhost:8000"; // Make sure this matches your API
...
const InspectionsDisplay = () => {
    ...
    useEffect(() => {
        const fetchInspections = async () => {
            try {
                const response = await fetch(`${apiUrl}/createinspection`);
    ...
    const filteredInspections = inspections.filter(inspection =>
        inspection.sitecode.toLowerCase().includes(searchTerm.toLowerCase()) // Search by site code
    );
