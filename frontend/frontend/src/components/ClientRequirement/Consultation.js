        ...
        <label className="form-check-label">
          ...
          <div className="row">
              ...
              <ul>
                {requirementsList.map((req, index) => (
                  <li key={index}>
                    ...
                    <h5>Project Name: {req.projectName}</h5>
                   <p><strong>Client Name:</strong> {req.clientName}</p>
                   <p><strong>Date of Consultation:</strong> {req.dateOfConsultation}</p>
                   <p><strong>Conducted By:</strong> {req.conductedBy}</p>
                   <p><strong>Project Goals:</strong> {req.projectGoals}</p>
                     <p><strong>Budget Constraints:</strong> Rs.{req.budgetConstraints}</p>
            
            {/* Aesthetic Preferences */}
            <h6>Aesthetic Preferences:</h6>
            <p><strong>Type of Building:</strong> {req.aestheticPreferences.typeOfBuilding}</p>
            <p><strong>Approved Type:</strong> {req.aestheticPreferences.approvedType}</p>
            <p><strong>Design Style:</strong> {req.aestheticPreferences.designStyle}</p>
            <p><strong>Roof Type:</strong> {req.aestheticPreferences.roofType}</p>
            <p><strong>Wall Material:</strong> {req.aestheticPreferences.wallMaterial}</p>
            <p><strong>Roof Material:</strong> {req.aestheticPreferences.roofMaterial}</p>
            <p><strong>Door/Window Material:</strong> {req.aestheticPreferences.doorWindowMaterial}</p>
            ...
            <ul>
              ...
              {req.selectedFloor === "firstFloor" && <li>Number of Bedrooms: {req.functionalRequirements.numberOfBedrooms}</li>}
            </ul>

            {/* Additional Information */}
            <h6>Additional Details:</h6>
            <p><strong>Regulatory Requirements:</strong> {req.regulatoryRequirements}</p>
            <p><strong>Sustainability Goals:</strong> {req.sustainabilityGoals}</p>
            <p><strong>Client Feedback:</strong> {req.clientFeedback}</p>
            <p><strong>Additional Notes:</strong> {req.additionalNotes}</p>

                    <button onClick={() => handleEdit(index)} className="btn btn-warning btn-sm ms-2">Edit</button>
                    <button onClick={() => handleDelete(index)} className="btn btn-danger btn-sm ms-2">Delete</button>
                  </li>
                ))}
              </ul>
            </div>
            <button onClick={handleBackToForm} className="btn btn-primary">Back to form </button>
          </div>
        )}
      </div>
    </>
